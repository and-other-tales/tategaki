...
class PageSizeSelector:
    ...
    def calculate_grid_dimensions(
        page_width, page_height, margins, character_size=None
    ):
        """
        ...
        if character_size is None:
            character_size = (
                6
                if text_width < 80
                else 7 if text_width < 100 else 8 if text_width < 150 else 9
            )
    ...
    def show_sizes(self):
        ...
        for i, size in enumerate(self.COMMON_SIZES, 1):
            ...
            table.add_row(
                str(i), size["name"], size_info, grid_info, size["description"]
            )
    ...
    def select_page_size(self):
        ...
        if selected["name"] == "Custom":
            ...
            bottom = int(
                Prompt.ask("Bottom margin (mm)", default="15", console=self.console)
            )
            inner = int(
                Prompt.ask("Inner margin (mm)", default="12", console=self.console)
            )
            outer = int(
                Prompt.ask("Outer margin (mm)", default="8", console=self.console)
            )
            ...
            self.console.print(
                f"[bold green]✓ Grid:[/bold green] {grid['columns']}×{grid['rows']} ({grid['characters_per_page']} characters/page)"
            )
        ...
        self.console.print(
            f"[bold green]✓ Grid:[/bold green] {selected['grid']['columns']}×{selected['grid']['rows']} ({selected['characters_per_page']} characters/page)"
        )

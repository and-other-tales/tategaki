...
# ░▀▄▀░▒█▒▒█▒█░█▄▄░█▀▄░▒█▒░█▀█▒█▄▄░█▄▄▒▄██
"""
Genkō Yōshi Tategaki Converter with AI-powered rule processing
...
try:
    ...
    print(
        "Install with: pip install langgraph langchain-anthropic langchain-huggingface"
    )
...
def create_llm_instance(
    ...
    if provider == "huggingface" and HUGGINGFACE_AVAILABLE:
        ...
        if not token:
            console.print(
                "[bold red]Warning: HF_TOKEN not found. Using public models only.[/bold red]"
            )
        ...
        try:
            ...
            if use_api:
                ...
                tokenizer = AutoTokenizer.from_pretrained(
                    model_name, use_auth_token=token if token else None
                )
            ...
            console.print(
                f"[green]✓ HuggingFace model initialized: {model_name}[/green]"
            )
            return chat_llm

        except Exception as e:
            console.print(
                f"[bold red]Error initializing HuggingFace model: {e}[/bold red]"
            )
    ...
    if provider == "anthropic" and ANTHROPIC_AVAILABLE:
        ...
        llm = ChatAnthropic(
            model=model_name, api_key=anthropic_key, temperature=0.1, max_tokens=4000
        )
    ...
    if not available_providers:
        raise ImportError(
            "No LLM providers available. Install langchain-anthropic or langchain-huggingface."
        )
...
if LANGGRAPH_AVAILABLE:
    ...
    class TextValidationResult(BaseModel):
        ...
        is_valid: bool = Field(
            description="Whether the text follows Japanese writing conventions"
        )
        corrections: List[str] = Field(description="List of suggested corrections")
        compliance_score: float = Field(
            description="Score from 0-1 for compliance with genkou yoshi rules"
        )
...
if LANGGRAPH_AVAILABLE:
    ...
    class AITextProcessor:
        ...
        def __init__(
            ...
            if not LANGGRAPH_AVAILABLE:
                raise ImportError(
                    "LangGraph and LLM providers are required for AI processing"
                )

            # Initialize LLM based on provider
            self.provider = provider or os.getenv("LLM_PROVIDER", "anthropic").lower()
            self.console.print(
                f"[cyan]Initializing LLM provider: {self.provider}[/cyan]"
            )
            ...
            try:
                ...
                self.console.print(
                    f"[bold red]Failed to initialize LLM: {e}[/bold red]"
                )
                raise

            # Create structured output models (with fallback for HuggingFace)
            try:
                self.validation_llm = self.llm.with_structured_output(
                    TextValidationResult
                )
                self.rule_processing_llm = self.llm.with_structured_output(
                    RuleProcessingResult
                )
                self.supports_structured_output = True
            except Exception as e:
                self.console.print(
                    f"[yellow]Warning: Structured output not supported by {self.provider}, using text parsing[/yellow]"
                )
        ...
        Provide specific, actionable feedback for improvements."""
            ...
            messages = [
                ...
                HumanMessage(
                    content=f"Please validate this Japanese text:\n\n{state['input_text']}"
                ),
            ]
            ...
            try:
                if self.supports_structured_output:
                    ...
                    self.console.print(
                        f"[cyan]Compliance score: {result.compliance_score:.2f}[/cyan]"
                    )
                else:
                    ...
                    self.console.print(f"[green]✓ Text validation completed[/green]")
                    self.console.print(
                        f"[cyan]Compliance score: {parsed_result['compliance_score']:.2f}[/cyan]"
                    )
        ...
        Return the processed text with all rules applied correctly."""
            ...
            messages = [
                SystemMessage(content=system_prompt),
                HumanMessage(
                    content=f"Apply genkou yoshi rules to this text:{validation_context}\n\nText to process:\n{state['input_text']}"
                ),
            ]
            ...
            try:
                if self.supports_structured_output:
                    ...
                    self.console.print(
                        f"[cyan]Confidence: {result.confidence:.2f}[/cyan]"
                    )
                    self.console.print(
                        f"[yellow]Rules applied: {len(result.rules_applied)}[/yellow]"
                    )
                else:
                    # Use text parsing for non-structured output
                    result = self.rule_processing_llm.invoke(messages)
                    ...
                    parsed_result = self._parse_rule_processing_response(
                        response_text, state["input_text"]
                    )
                    ...
                    self.console.print(f"[green]✓ Rule processing completed[/green]")
                    self.console.print(
                        f"[cyan]Confidence: {parsed_result['confidence']:.2f}[/cyan]"
                    )
                    self.console.print(
                        f"[yellow]Rules applied: {len(parsed_result['rules_applied'])}[/yellow]"
                    )
        ...
        def _finalize_text_node(self, state: ProcessingState) -> ProcessingState:
            ...
            self.console.print(
                f"[bold green]✓ AI processing pipeline completed[/bold green]"
            )
        ...
        def _parse_validation_response(self, response_text: str) -> Dict:
            ...
            score_match = re.search(
                r"(?:compliance|score)[:\s]*([0-9.]+)", response_text.lower()
            )
            ...
            for line in lines:
                ...
                if any(word in line.lower() for word in ["correction", "fix", "error"]):
                    ...
                    word in line.lower()
                    for word in ["suggestion", "recommend", "improve"]
                ):
        ...
        def _parse_rule_processing_response(
            ...
            if processed_text == original_text:
                ...
                japanese_blocks = re.findall(
                    r"[ぁ-んァ-ヶ一-龯]+[ぁ-んァ-ヶ一-龯。、！？「」]*", response_text
                )
                if japanese_blocks:
                    # Use the longest Japanese block
                    processed_text = max(
                        japanese_blocks, key=len, default=original_text
                    )

            # Extract confidence
            confidence_match = re.search(
                r"(?:confidence|certainty)[:\s]*([0-9.]+)", response_text.lower()
            )
            ...
            for line in lines:
                ...
                if any(
                    word in line.lower() for word in ["rule", "applied", "correction"]
                ):
                    if line.startswith(("•", "-", "*", "1.", "2.", "3.")):
                        content = re.sub(r"^[•\-*0-9.]\s*", "", line)
                        rules_applied.append(content)
                elif any(
                    word in line.lower() for word in ["violation", "error", "issue"]
                ):
                    if line.startswith(("•", "-", "*", "1.", "2.", "3.")):
                        content = re.sub(r"^[•\-*0-9.]\s*", "", line)
        ...
        def process_text(
            self, text: str, page_format: Dict, thread_id: str = "default"
        ) -> Dict:
            ...
            self.console.print(
                "[bold cyan]Starting AI-powered text processing...[/bold cyan]"
            )
    ...
    class AITextProcessor:
        def __init__(
            ...
            self.console = console
            self.console.print(
                "[bold red]LangGraph not available - AI processing disabled[/bold red]"
            )
...
class EnhancedGenkouYoshiDocumentBuilder(GenkouYoshiDocumentBuilder):
    ...
    def __init__(
        ...
        squares_per_column=None,
        max_columns_per_page=None,
        ...
        self.llm_provider = (
            llm_provider or os.getenv("LLM_PROVIDER", "anthropic").lower()
        )
        ...
        if use_ai and LANGGRAPH_AVAILABLE:
            try:
                ...
                self.console.print(
                    f"[bold green]✓ AI processing enabled with {self.llm_provider}[/bold green]"
                )
            except Exception as e:
                self.console.print(
                    f"[bold red]✗ AI processing setup failed: {e}[/bold red]"
                )
                self.use_ai = False
        elif use_ai:
            self.console.print(
                "[bold red]✗ AI processing requires LangGraph and LLM providers[/bold red]"
            )
            self.use_ai = False
        ...
        try:
            ...
            if self.use_ai and self.ai_processor:
                ...
                if ai_result.get("validation_result"):
                    ...
                    self.console.print(
                        f"[green]AI Validation Score: {validation.get('compliance_score', 'N/A')}[/green]"
                    )

                if ai_result.get("rule_processing_result"):
                    rule_result = ai_result["rule_processing_result"]
                    self.console.print(
                        f"[green]AI Processing Confidence: {rule_result.get('confidence', 'N/A')}[/green]"
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Japanese Tategaki DOCX Generator with AI Enhancement"
    )
    ...
    parser.add_argument(
        "--skip-verification", action="store_true", help="Skip verification process"
    )
    parser.add_argument(
        "--verification-report", help="Save verification report to file"
    )

    # AI processing options
    parser.add_argument(
        "--ai", action="store_true", help="Enable AI-powered rule processing"
    )
    parser.add_argument(
        "--llm-provider",
    ...
    parser.add_argument(
        "--ai-thread-id",
        default="default",
        help="Thread ID for AI conversation context",
    )
    ...
    console.print(
        "[green]Genkou Yoshi formatting with LangGraph + Multi-LLM support[/green]"
    )

    if args.ai:
        provider = args.llm_provider or os.getenv("LLM_PROVIDER", "anthropic").lower()
        console.print(
            f"[bold cyan]🤖 AI Processing Mode Enabled ({provider})[/bold cyan]"
        )
    console.print()
    ...
    if not input_path.exists():
        console.print(
            f"[bold red]Error: Input file '{input_path}' not found.[/bold red]"
        )
        sys.exit(1)
    ...
    if args.ai and not LANGGRAPH_AVAILABLE:
        console.print(
            "[bold red]Error: AI processing requires LangGraph and LLM providers.[/bold red]"
        )
        console.print(
            "Install with: pip install langgraph langchain-anthropic langchain-huggingface"
        )
    ...
    if args.ai:
        ...
        if provider == "anthropic":
            if not ANTHROPIC_AVAILABLE:
                console.print(
                    "[bold red]Error: Anthropic provider not available.[/bold red]"
                )
            ...
            if not (args.anthropic_api_key or os.getenv("ANTHROPIC_API_KEY")):
                ...
                console.print(
                    "Set --anthropic-api-key or ANTHROPIC_API_KEY environment variable"
                )
                sys.exit(1)
            ...
            if not HUGGINGFACE_AVAILABLE:
                console.print(
                    "[bold red]Error: HuggingFace provider not available.[/bold red]"
                )
                console.print(
            ...
            if not (
                ...
                console.print(
                    "[bold yellow]Warning: No HF_TOKEN found. Using public models only.[/bold yellow]"
                )
    ...
    if args.format is None or args.format == "custom":
        if PageSizeSelector and Prompt:
            ...
            console.print(
                "[bold red]Error: Interactive page size selection requires 'rich.prompt' and 'sizes.py'.[/bold red]"
            )
            sys.exit(1)
    else:
        try:
            page_format = (
                PageSizeSelector.get_format(args.format) if PageSizeSelector else None
            )
        except Exception:
            page_format = None
    ...
    with Progress(
        ...
        if args.output:
            ...
            format_name = (
                page_format.get("name", "genkou_yoshi")
                if page_format
                else "genkou_yoshi"
            )
        ...
        progress.update(
            task, advance=30, description="Document saved, preparing verification..."
        )
    ...
    if not args.skip_verification:
        ...
        if args.verification_report:
            ...
            console.print(
                f"[bold green]✓ Verification report saved:[/bold green] {args.verification_report}"
            )
        ...
        if verification_report["status"] == "compliant":
            ...
            console.print(
                f"[green]Achieved in {verification_report['iterations']} iteration(s)[/green]"
            )
            ...
            console.print(
                f"[yellow]Compliance score: {verification_report['final_score']}/100[/yellow]"
            )
            console.print(
                f"[yellow]Remaining violations: {verification_report['remaining_violations']}[/yellow]"
            )
        else:
    ...
    if args.ai:
        provider = args.llm_provider or os.getenv("LLM_PROVIDER", "anthropic").lower()
        console.print(
            f"[bold cyan]🤖 AI processing completed with {provider}[/bold cyan]"
        )
